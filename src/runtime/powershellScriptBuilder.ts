  ...
  const content = [
    ...
    ...(input.mode === "resume"
      ? [
          ...
          `    Write-TuiLog "Attempt $attempt of \${maxAttempts}: $CodexCommand $($CodexArgs -join ' ')"`,
          ...
          `    if ($exitCode -eq 0 -or $elapsedSeconds -ge $minSuccessfulRunSeconds) {`,
          `      break`,
          `    }`,
          `    if ($attempt -lt $maxAttempts) {`,
          `      Write-TuiLog "Thread may not have a rollout yet; retrying in $retryDelaySeconds s..."`,
          `      Start-Sleep -Seconds $retryDelaySeconds`,
          `    } else {`,
          `      Write-TuiLog "Giving up after $maxAttempts attempts."`,
          `    }`,
          `  }`,
        ]
      : [
